...
for j=1:6
  for k=1:3
    for i=1:2
      ...
      learnN[j,k,i] = (1+(1-eta)*x/sum(repmat(resArray[1,j,k].PP[1].L[1,:]/5,1,5).*resArray[1,j,k].c[1,:,:,1].^(1-eta)))^(1/(1-eta))-1
    end
  end
end

learn = DataFrame(Model = repmat([-1; 0; 1]',6,1)[:], Regime = repmat(regimes,3), l2015lto2045=learnN[:,:,1][:], l2045tolend =learnN[:,:,2][:])
writetable("$(pwd())/Outputs/valueOfLearning/valueOfLearningNICE.csv",learn)
save("$(pwd())/Outputs/valueOfLearning/resArrayNICE.jld", "resArray", resArray)
# learnD = zeros(5,2)
# for j=1:5
#   for k=1:2
