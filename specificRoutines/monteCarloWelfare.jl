...
eeMv = [-1] #[0 1] # elasticity of damage with respect to income
model="NICE"
regimes = [9 16 18 19] #[18] #[9, 16] #, 19
lmv = [0 3 tm]
#compute optima
# pre allocate the dataframes and results array
resArray = Array{Results}(length(eeMv),length(regimes),length(lmv),20)
initials = Array{Vector{Float64}}(length(eeMv),length(regimes),length(lmv),20)
  ...
  for (h,regime) in enumerate(regimes)
    ...
    for (i, lm) in enumerate(lmv)
      for j=1:8
        ...
        writetable("$(pwd())/Outputs/valueOfLearning/welfaresxi10m1.csv",WELF)
